...
import { type DayMenu, restOfWeek } from "./matkant";
...
const [SEND_HOUR = 9, SEND_MINUTE = 0] = (process.env.SEND_AT ?? "09:00")
	.split(":")
	.map(Number);
...
async function getDaysTranslated(
	lang: Language,
): Promise<[DayMenu[], Translations]> {
	const days = await getDayMenu();
	return [
		days,
		await translateAll(textsToTranslate(days, lang), locale(lang).translateTo),
	];
}
...
bot.command("start", async (ctx) => {
	...
	await ctx.reply(
		`${locale(lang).ui.alreadySubscribed}\n\n${helpText(lang, ctx.from?.id)}`,
		send,
	);
});
...
bot.command(allNames(COMMANDS.evening), async (ctx) => {
	...
	const tr = await translateAll(
		textsToTranslateEvening(weeks, lang),
		locale(lang).translateTo,
	);
...
bot.command(allNames(COMMANDS.admin), async (ctx) => {
	...
	const lines = [
		...
		...[...bylang].map(
			([lang, n]) => `  • ${LANGUAGE_NAMES[lang as Language] ?? lang}: ${n}`,
		),
		"",
		`<b>${ui.adminSubsListLabel}</b>`,
		...(subs.length > 0
			? subs.map(
					(s) => `<code>${s.chatId}</code> - ${s.language} - ${s.createdAt}`,
				)
			: [`  ${ui.adminNone}`]),
		"",
		...
		...(cacheStats.size > 0
			? [...cacheStats].map(
					([lang, n]) => `  • ${lang}: ${n} ${ui.adminCacheEntrySuffix}`,
				)
			: [`  ${ui.adminCacheEmpty}`]),
	];
...
function commandList(
	...
	const list = [
		...
		{
			command: COMMANDS.status.primary,
			description: ui.statusSubscriptionLabel,
		},
	];
	if (admin)
		list.push({
			command: COMMANDS.admin.primary,
			description: ui.cmdAdminDesc,
		});
...
await bot.api.setMyCommands([
	{
		command: COMMANDS.today.primary,
		description: "Today's menu and upcoming days",
	},
	...
	{
		command: COMMANDS.stop.primary,
		description: "Unsubscribe from the daily menu",
	},
]);
